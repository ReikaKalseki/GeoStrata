...
public class CreepvineGenerator implements RetroactiveGenerator {
	...
	public boolean generate(World world, int x, int y, int z, Random rand, int minHeight, int minHeightFertile, float fertileChance, boolean growFertile) {
		int y1 = y;
		...
		if (y1 < 60 && y2 < 64 && BlockCreepvine.canGrowOn(world, x, y1-1, z)) {
			int diff = y2-y1;
			if (diff >= 8 && diff < 20) {
				int h = ReikaRandomHelper.getRandomBetween(minHeight, Math.min(diff-1, 16), rand);
				y2 = y1+h;
				...
				if (fertile) {
					...
					world.setBlock(x, core, z, GeoBlocks.CREEPVINE.getBlockInstance(), (growFertile ? Pieces.CORE_5 : Pieces.CORE_EMPTY).ordinal(), 2);
